"""
Training script for flow matching pulse denoiser.

Usage:
    python -u -m src.flowMatching.train \
        --clean_dir /media/AVFD/yunshancheng/cuore/clean \
        --noise_dir /media/AVFD/yunshancheng/cuore/noise \
        --output_dir /media/AVFD/yunshancheng/cuore/flow_test \
...
def main():
    parser = argparse.ArgumentParser(description="Train flow matching pulse denoiser.")
    ...
    parser.add_argument('--steps', type=int, default=50,
                        help='Number of ODE integration steps (for config logging)')
    ...
    if args.scale_cond:
        ...
        print("Using scale-conditioned U-Net (FlowUNet1DScaleCond)")
    else:
    ...
    flow = FlowMatching(model, loss_type=args.loss, spectral_loss=spec_loss)
    ...
    if args.resume:
        print(f"Cosine schedule: {args.lr:.2e} -> 1e-7 over {remaining_epochs} epochs")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for x_clean, x_noisy, batch_scale in train_loader:
            ...
            with autocast_ctx():
                result = flow.training_loss(x_clean, x_noisy, scale=sc)
                if use_spectral:
